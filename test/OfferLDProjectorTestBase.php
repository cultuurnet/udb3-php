...
abstract class OfferLDProjectorTestBase extends \PHPUnit_Framework_TestCase
{
     ...
     * @test
     */
    public function it_should_remove_all_existing_openingshours_when_we_project_new_major_info_without_hours()
    {
        $this->markTestIncomplete('The actual update has been moved to an update factory');

        $itemId = 'foo';
        $eventClass = $this->getEventClass('MajorInfoUpdated');
        $majorInfoUpdatedEvent = new $eventClass(
            $itemId,
            new Title('title'),
        ...
        $this->documentRepository->save($initialDocument);
        $projection = $this->project($majorInfoUpdatedEvent, $itemId);
        $this->assertEquals($expectedProjection, $projection);
    }
}
